<?php
	...
	/* General variables */
	$filename = "websstat/db/".date("m-Y").".txt";
	$file = preg_replace("/websstat\/db\//i", "", $filename);
	$file = preg_replace("/-/i", "", $file);
	$file = substr($file, 0, -4);

	/* Checks if the current month file exists */
	if (date("mY") <= $file) {
		...
		if (file_exists($filename)) {
			...
			if (is_writable($filename)) {
				...
				if ($handle = fopen($filename, "a")) {
					...
					if (!empty($_SERVER["HTTP_REFERER"])) {
						...
						$referer = "https://test.webss.ro/"; // URL's site
					}
					...
					foreach($html->find(".data-item") as $element) {
						if (strpos($element->plaintext, "Country") !== false) {
							$preg_country = preg_replace("/\s+/","", $element->plaintext);
								
							if (!empty($preg_country)) {
								$country = substr($preg_country,8);
							} else {
								$country = "nd";
							}
					...
					foreach($html->find(".data-item") as $element) {
						if (strpos($element->plaintext, "City") !== false) {
							$preg_city = preg_replace("/\s+/","", $element->plaintext);
								
							if (!empty($preg_city)) {
								$city = substr($preg_city,5);
							} else {
								$city = "nd";
							}
					...
					if (filesize("websstat/db/".date("m-Y").".txt") <= 2) {
			...
			$next = "websstat/db/".date("m-Y", strtotime("+1 month")).".txt";
		...
		/* If the current month file does not exist, create it */
		$handle = fopen($filename, "w+");
		fwrite($handle, "");
		fclose($handle);
	}
